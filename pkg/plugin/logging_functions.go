...
func (o *OCIDatasource) processLogMetricTimeSeries(ctx context.Context,
	...
	o.logger.Error("processLogMetricTimeSeries", "queryModel", fmt.Sprintf("%v", queryModel))
	o.logger.Error("processLogMetricTimeSeries", "mFieldDefns", fmt.Sprintf("%v", mFieldDefns))
	...
	o.logger.Error("searchLogsReq", "searchLogsReq", searchLogsReq.SearchQuery)
	o.logger.Error("Processing log metrics time series search query", "panelId", queryPanelId, "refId", queryRefId,
	...
	o.logger.Error("start", "start", start)
	o.logger.Error("end", "end", end)
	...
	o.logger.Error("req1", "req1", fmt.Sprintf("%v", req1))
	...
	o.logger.Error("request", "request", fmt.Sprintf("%v", request))
	reg := common.StringToRegion(queryModel.Region)
	o.logger.Error("request", "reg", reg)
	...
	o.logger.Error("processLogMetricTimeSeries", "res", fmt.Sprintf("%v", res.RawResponse.StatusCode))
	if err != nil {
	...
	//o.logger.Error("resultCount", "resultCount", fmt.Sprintf("%v", resultCount))
	if resultCount > 0 {
		...
		//o.logger.Error("searchResultData", "searchResultData", fmt.Sprintf("%v", searchResultData))
		if ok == true {
			//o.logger.Error("searchResultData[LogSearchResultsField_LogContent]", "searchResultData[LogSearchResultsField_LogContent]", fmt.Sprintf("%v", searchResultData[LogSearchResultsField_LogContent]))
			if _, ok := searchResultData[constants.LogSearchResultsField_LogContent]; !ok {
					...
					if ok == true {
						if timestampFloat, ok := searchResultData[timestampFieldKey].(float64); ok {
							...
							if _, ok = mLogTimeSeriesResults[timestampMs]; !ok {
								//o.logger.Error(fmt.Sprintf("Results timestamp grouping does NOT exist: %d", timestampMs))
				...
				//o.logger.Error("tgtNumRows", "tgtNumRows", fmt.Sprintf("%v", tgtNumRows))
				...
				for rowCount, timestampMs := range sTimestampKeys {
					...
					//o.logger.Error("mFieldDefns2", "mFieldDefns2", fmt.Sprintf("%v", mFieldDefns))
					//o.logger.Error("fieldDefn", "fieldDefn", fmt.Sprintf("%v", fieldDefn))
					...
					for _, searchResultFieldsPtr := range timestampResults.mMetricResults {
						...
						if numericFieldType == constants.ValueType_Float64 {
							...
							//o.logger.Error("mFieldDefns3", "mFieldDefns3", fmt.Sprintf("%v", mFieldDefns))
							//o.logger.Error("fieldDefn", "fieldDefn", fmt.Sprintf("%v", fieldDefn))
							...
							//o.logger.Error("mFieldDefns4", "mFieldDefns4", fmt.Sprintf("%v", mFieldDefns))
							//o.logger.Error("fieldDefn", "fieldDefn", fmt.Sprintf("%v", fieldDefn))
					...
					rowCount++
					//o.logger.Error("rowCount", "rowCount", fmt.Sprintf("%v", rowCount))
				}
...
func (o *OCIDatasource) processLogRecords(ctx context.Context,
	...
	var numpage = 1
	var indexCountPag = 0
	o.logger.Error("processLogRecords", "processLogRecords", "processLogRecords")
	...
	for res, err := o.tenancyAccess[takey].loggingSearchClient.SearchLogs(ctx, request); ; res, err = o.tenancyAccess[takey].loggingSearchClient.SearchLogs(ctx, request) {
		...
		o.logger.Error("res", "res", res.RawResponse.StatusCode)
