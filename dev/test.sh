#!/usr/bin/env bash
#
# OCI Logs Plugin — compatibility test runner
#
...
for arg in "$@"; do
    case "$arg" in
        --help|-h)
            ...
            echo ""
            echo "Tests the OCI Logs plugin across Grafana v7.5, v9, v10, v11, v12."
            echo ""
            echo "Options:"
...
echo -e "${BOLD}▶ yarn build${NC} (full log: /tmp/test-yarn.log)"
yarn build 2>&1 | tee /tmp/test-yarn.log
echo ""

echo -e "${BOLD}▶ mage -v${NC} (full log: /tmp/test-mage.log)"
mage -v 2>&1 | tee /tmp/test-mage.log
echo ""

# --- Start containers ---
echo -e "${BOLD}Starting Grafana containers...${NC} (full log: /tmp/test-compose.log)"
cd "$DEV_DIR"
docker compose up -d 2>&1 | tee /tmp/test-compose.log
echo ""
...
# --- Print browser-accessible URLs (Cmd+Click in most terminals) ---
echo ""
echo -e "${BOLD}Open in browser:${NC} (anonymous Admin enabled)"
printf "  %-12s %s\n" "Grafana 7.5" "http://localhost:3075"
printf "  %-12s %s\n" "Grafana 9"   "http://localhost:3090"
printf "  %-12s %s\n" "Grafana 10"  "http://localhost:3100"
printf "  %-12s %s\n" "Grafana 11"  "http://localhost:3110"
printf "  %-12s %s\n" "Grafana 12"  "http://localhost:3120"
echo ""
echo -e "  ${BOLD}Test dashboard${NC} (replace PORT):  http://localhost:PORT/d/oci-logs-test"
echo -e "  ${BOLD}Datasource list${NC} (replace PORT): http://localhost:PORT/connections/datasources"
